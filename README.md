...
.venv/Scripts/python.exe -m pip install openpyxl wechat-search-skill
```

### 2. 安装 / 检查项目级 Hermes 插件

本 repo 已包含项目级插件配置。需要从项目启动入口进入 Hermes，确保项目插件被加载：

- Windows：双击 `start-hermes-project-plugins.bat`
- Bash：运行 `./start-hermes-project-plugins.sh`

### 3. 检查 MCP 是否可用

在 Hermes 中 reload MCP 后，确认以下工具可见：

- `dsh-wechat-mp-search`：`weixin_search`、`weixin_search_all`、`get_weixin_article_content`
- `mcp-webscraper`：`scrape_url`、`extract_data`、`extract_first`、`batch_scrape`、`crawl_website`

### 4. 重新生成 Excel 结果

```bash
python create_result_python/create_A_result_1.py
python create_result_python/create_A_result_2.py
python create_result_python/create_B_result.py
```
...
- mcp-webscraper：https://github.com/samirsaci/mcp-webscraper
- dsh-wechat-mp-search npm：https://www.npmjs.com/package/dsh-wechat-mp-search
- dsh-wechat-mp-search GitHub：https://github.com/iTraceur/dsh-wechat-mp-search
- weixin-public-account-skill / wechat-search：https://github.com/qbu11/weixin-public-account-skill
- wechat-article-spider：https://github.com/qbu11/wechat-article-spider
